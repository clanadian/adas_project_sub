#!/usr/bin/env python3
"""Pure-integer bit-exact golden model for the arty_96_classifier prototype
— the actual hardware arithmetic (int8 x int8 -> int32 accumulate, +int32
bias, leaky 13/128 on the accumulator, int64 requant with round_shift,
saturate to [-128,127]), not a float approximation.

Mirrors apply_activation() in conv_engine.cpp:76 / conv0_engine.cpp:47. The
sibling roi_classifier_int8_export/ implements the OTHER prototype (ReLU,
truncating shift, [0,127]); do not cross-check one against the other. Used to (a) produce the
...
from fixed_point import apply_activation, round_half_away_from_zero, round_shift

EXPORT_DIR = Path("/mnt/d/fpga_project/roi_classifier_int8_export_arty96/export")
...
def requant_activate(acc_int64: np.ndarray, multiplier: int, shift: int, leaky: bool) -> np.ndarray:
    """conv_engine.cpp apply_activation(): leaky on the accumulator (conv0/1
    only), then round_shift requant, then saturate to signed INT8."""
    return apply_activation(acc_int64, leaky, multiplier, shift)
...
def run_golden(export: dict, img_rgb_u8: np.ndarray) -> dict:
    ...
    cur = x
    # leaky on conv0/conv1, conv2 linear — matches classifier_net.h's op table
    # (all three of gen_golden.c / verify_golden.py / classifier_run.c agree
    # conv2's leaky flag is 0).
    for name, pad, leaky in [("conv0", 0, True), ("conv1", 1, True), ("conv2", 1, False)]:
        L = export["layers"][name]
        acc = int8_conv3x3(cur, L["weight"], L["bias"], pad=pad)
        out = requant_activate(acc, L["multiplier"], L["shift"], leaky)
