...
typedef struct {
    ...
    XConv0_engine_uio_map maps[ MAX_UIO_MAPS ];
} XConv0_engine_uio_info;

/***************** Variable Definitions **************************************/
static XConv0_engine_uio_info uio_info;
...
static int uio_info_read_name(XConv0_engine_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConv0_engine_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConv0_engine_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConv0_engine_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConv0_engine_Initialize(XConv0_engine *InstancePtr, const char* InstanceName) {
	XConv0_engine_uio_info *InfoPtr = &uio_info;
