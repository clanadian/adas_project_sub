;# arty_ps7_preset_z7_20.tcl - Digilent Arty Z7-20 PS7 preset, applied explicitly.
;#
;# Zybo Z7-20 판(zybo_ps7_preset_z7_20.tcl)과 같은 이유로 존재한다: 보드 파일을
;# Vivado 에 설치하지 않고 프리셋 값만 명시 적용해 빌드를 자체 완결시킨다.
;# 그 파일의 머리 주석이 근거의 정본이고, 여기서는 **Arty 가 Zybo 와 다른 점**만
;# 적는다.
;#
;#   항목            Arty Z7-20            Zybo Z7-20
;#   파트            xc7z020clg400-1       xc7z020clg400-1   <- 같다 (PL 무변경)
;#   DDR 버스 폭     16 bit                32 bit            <- 대역폭 절반
;#   DDR 용량        512 MB                1 GB              <- 이 망은 15.8 MB 만 쓴다
;#   DDR 타입        DDR 3                 DDR 3 (Low Voltage)
;#   콘솔            UART0, MIO 14..15     UART1, MIO 48..49  <- PS 인계 사항
;#   크리스탈        50 MHz                33.333333 MHz
;#
;# ⚠️ DDR 버스 폭 16 bit 은 **실보드에서만 드러나는 위험**이다. 이 저장소의
;#    사이클 수치는 전부 cosim 이고 cosim 은 이상적 AXI 슬레이브를 쓴다 -
;#    DDR 대역폭을 모델링한 적이 없다. AXI 캠페인 결론("비용은 트랜잭션 개수가
;#    아니라 루프 반복에 붙어 있다")이 민감도가 낮음을 시사하지만 그것도 cosim
;#    이야기다. **Arty 는 이 축에서 Zybo 보다 엄격히 불리하다.**
;#
;# ⚠️⚠️ **순서가 여기서는 진짜로 필수다.** 이 프리셋은
;#    **PCW_USE_S_AXI_HP0 = 0 으로 HP0 을 끈다.** 이 설계는 m_axi 마스터 9개가
;#    전부 HP0 으로 들어가므로 HP0 이 꺼지면 BD 가 성립하지 않는다.
;#    빌드 스크립트가 이 파일을 source 한 **뒤에** HP0=1 / 폭 64 를 다시 넣기
;#    때문에 설계 값이 이긴다. Zybo 판에서는 이 순서가 보험이었지만
;#    (겹치는 항목이 같은 값 하나뿐) Arty 에서는 부하가 걸려 있다.
;#    프리셋을 나중에 적용하도록 순서를 바꾸면 조용히 깨진다.
;#
;# 타이밍: 이 프리셋은 PCW_FPGA0_PERIPHERAL_FREQMHZ 를 100 으로 **직접 설정**한다
;# (Zybo 프리셋은 설정하지 않았다). 설계도 같은 값을 뒤에서 다시 넣으므로
;# 결과는 같지만, BD_ONLY 게이트가 FCLK0==100 을 실제로 확인한다.
;#
;# SOURCE: https://github.com/Digilent/vivado-boards (MIT), commit 36f34ab
;#         new/board_files/arty-z7-20/*/preset.xml (884 parameters)
;# GENERATED - do not hand-edit. Regenerate from the upstream preset instead.

if {[llength [get_bd_cells -quiet ps7_0]] == 0} {
    puts "== FATAL: ps7_0 not found."
    exit 1
}

puts "== Applying Digilent Arty Z7-20 PS7 preset (884 parameters)"
...
set_property -dict [list \
    ...
    CONFIG.C_USE_GPI2                              {1} \
    CONFIG.C_GPI2_SIZE                             {2} \
    CONFIG.C_USE_GPI3                              {1} \
    CONFIG.C_GPI3_SIZE                             {2} \
    CONFIG.C_USE_GPI4                              {1} \
    CONFIG.C_GPI4_SIZE                             {2} \
    CONFIG.USE_GPI1                                {1} \
    CONFIG.GPI1_SIZE                               {2} \
    CONFIG.USE_GPI2                                {1} \
    CONFIG.GPI2_SIZE                               {2} \
    ...
    CONFIG.C_ALL_OUTPUTS_2                         {0} \
    CONFIG.C_USE_GPI1                              {1} \
    CONFIG.C_GPI1_SIZE                             {4} \
    CONFIG.C_USE_GPI2                              {1} \
    CONFIG.C_GPI2_SIZE                             {4} \
    CONFIG.C_USE_GPI3                              {1} \
    CONFIG.C_GPI3_SIZE                             {4} \
    CONFIG.C_USE_GPI4                              {1} \
    CONFIG.C_GPI4_SIZE                             {4} \
    CONFIG.USE_GPI1                                {1} \
    CONFIG.GPI1_SIZE                               {4} \
    CONFIG.USE_GPI2                                {1} \
    CONFIG.GPI2_SIZE                               {4} \
    ...
    CONFIG.C_USE_GPI1                              {1} \
    CONFIG.C_GPI1_SIZE                             {6} \
    CONFIG.C_USE_GPI2                              {1} \
    CONFIG.C_GPI2_SIZE                             {6} \
    CONFIG.C_USE_GPI3                              {1} \
    CONFIG.C_GPI3_SIZE                             {6} \
    CONFIG.C_USE_GPI4                              {1} \
    CONFIG.C_GPI4_SIZE                             {6} \
    CONFIG.USE_GPI1                                {1} \
    CONFIG.GPI1_SIZE                               {6} \
    CONFIG.USE_GPI2                                {1} \
    CONFIG.GPI2_SIZE                               {6} \
...
;# 부팅을 좌우하는 값만 되읽어 확인한다. Arty 는 콘솔이 UART0 이다.
foreach {p want} {
    CONFIG.PCW_UIPARAM_DDR_PARTNO       {MT41J256M16 RE-125}
    CONFIG.PCW_SD0_PERIPHERAL_ENABLE    {1}
    CONFIG.PCW_UART0_PERIPHERAL_ENABLE  {1}
} {
    set got [get_property $p [get_bd_cells ps7_0]]
    if {$got ne $want} {
        puts "== FATAL: $p = '$got', expected '$want'"
        exit 1
    }
    puts "==   OK $p = $got"
}
