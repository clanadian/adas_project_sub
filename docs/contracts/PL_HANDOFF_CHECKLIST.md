...
현재 접수·배포하는 정본은 `arty/pl_db/`, `arty/models/roi_classifier_int8_db/`,
`arty/classifier_linux_db/`의 최종 DB 구현이다.
...
활성화·시프트·클램프를 문서에 적는다. 가중치는 DB 산술에 맞춰
양자화돼 있으므로 다른 산술 구현에 재사용하지 않는다.

| 항목 | DB 정본 |
| --- | --- |
| 활성화 | ReLU (clamp 하한 0) |
| 시프트 | `scaled >> shift`, 라운딩 없음 |
| 출력 범위 | `[0, 127]` |
