...
- **rootfs 정식화** — 현재 initramfs라 재부팅하면 IP·가중치·서버 바이너리가 사라진다.
  서버 바이너리를 bitbake 레시피로 편입하고 영속 rootfs로 전환한다.
...
- **PL 구현 단일화** — 부록 A 참조.
- **EB 계열 실보드 연동** — PS 구현(3-IP 주소맵, 6-op 시퀀서, 커널 드라이버,
  단계별 golden 테스트)은 완료했고 실보드 검증이 남아 있다.

---

## 부록 A. PL 내부 구현 후보 (내부 사항)

외부 계약(입출력 형상·프로토콜·PS 구조)은 완전히 동일하지만, PL 내부 구조가 다른
두 구현을 병행 개발해 비교 중이며 **효율이 좋은 쪽으로 단일화할 예정**이다.
