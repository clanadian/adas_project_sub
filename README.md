...
두 번의 장시간 측정이 있고 둘 다 오류 0건이다. 조건이 다르므로 인용할 때
날짜를 함께 적는다.

| | 2026-08-21 | 2026-08-24 |
|---|---:|---:|
| 완료 프레임 / ROI | 70,095 / 70,719 | 15,268 / 33,634 |
| 프레임당 ROI 평균 | 1.01 | 2.20 |
| proposal 중앙값 | `17.35 ms` | `25.36 ms` |
| Jetson↔Arty TCP 왕복 | `8.24 ms` | `8.26 ms` |
| Arty PL 실행 | `6.62 ms` | `6.61 ms` |
| Arty 서버 전체 | `6.88 ms` | `6.88 ms` |
| 결과 갱신 속도 | `28.58 FPS` | `21.56 FPS` |
| 분류 오류 | 0 | 0 |

**PL 실행과 TCP 왕복은 두 측정에서 같다.** 전체 FPS 차이는 주로 장면의 ROI
개수 차이(1.01 → 2.20개)에서 온다. ROI 를 한 건씩 순차 전송하는 구조라
ROI 가 하나 늘 때마다 약 8.8 ms 가 더해진다.

2026-08-24 에는 실제 객체 5개 프레임 3,511개에서 **N=5, 14.27 FPS** 를
실측했고, N=10 은 실측 기반 환산으로 **약 8.8 FPS** 다(실측 아님). 같은 날
Arty 서버 세션 3회(21,291 / 24,340 / 22,264 건)도 모두 오류 0건이다.

proposal 중앙값이 `17.35 ms` 에서 `25.36 ms` 로 늘어난 원인은 확인되지
않았다. 두 측정 모두 Jetson Nano 5W 모드였고 PL 은 바뀌지 않았다. 상세는
[`docs/reports/SHUTDOWN_LOG_2026-08-24.md`](docs/reports/SHUTDOWN_LOG_2026-08-24.md)
§6, ROI 개수별 값은
[`docs/reports/ROI_COUNT_SCALING_REPORT_2026-08-24.md`](docs/reports/ROI_COUNT_SCALING_REPORT_2026-08-24.md)
를 본다.
